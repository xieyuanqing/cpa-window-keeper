...
docker run --rm --network none --cpuset-cpus "$CPU" \
  ...
  sh -ec 'go test -p 1 -race -count=1 ./...; go vet ./...; mkdir -p dist; go build -p 1 -trimpath -buildmode=c-shared -ldflags="-s -w" -o dist/cpa-window-keeper-v0.1.4.so .'
sha256sum dist/cpa-window-keeper-v0.1.4.so
